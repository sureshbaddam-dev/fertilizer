...
export const getSupplierLedger = asyncHandler(async (req, res) => {
  const data = await supplierService.getSupplierLedger(req.params.id, req.query);
...
export const recordSupplierPayment = asyncHandler(async (req, res) => {
  const data = await supplierService.recordSupplierPayment(req.params.id, req.body);
...
export const deletePayment = asyncHandler(async (req, res) => {
  ...
  const result = await supplierService.softDeletePayment(req.params.id, req.user || {}, confirmation);
  return sendSuccess(res, result.message, result, HTTP_STATUS.OK);
});

export const restorePayment = asyncHandler(async (req, res) => {
  const result = await supplierService.restorePayment(req.params.id, req.user || {});
  return sendSuccess(res, result.message, result, HTTP_STATUS.OK);
});
