...
import { getItemUnitPrice, formatItemDiscount } from '../../utils/pricing';
...
export default function PrintableInvoice({ invoice, shopSettings = {}, className = '' }) {
  ...
  const subtotal = Number(invoice.subtotal || rawSubtotal || invoice.totalAmount || 0);
        ...
        <div className="w-64 sm:w-72 rounded-xl border border-slate-300/80 bg-[#f8fafc] p-3.5 space-y-1.5 text-xs">
          ...
          {productDiscount > 0 && (
            <div className="flex justify-between items-center text-red-600">
              <span className="font-normal">Product Discount:</span>
              <span className="font-bold">- {formatCurrency(productDiscount)}</span>
            </div>
          )}

          {discountAmount > 0 && productDiscount <= 0 && billDiscount <= 0 && (
            <div className="flex justify-between items-center text-red-600">
          ...
          {billDiscount > 0 && (
            <div className="flex justify-between items-center text-red-600">
              <span className="font-normal">Bill Discount:</span>
              <span className="font-bold">- {formatCurrency(billDiscount)}</span>
            </div>
          )}

          <div className="flex justify-between items-center text-gray-700">
            <span className="font-normal">Taxable Amount:</span>
            <span className="font-bold text-gray-950">{formatCurrency(taxableAmount)}</span>
          </div>

          {taxAmount > 0 && (
            <div className="flex justify-between items-center text-gray-700">
              <span className="font-normal">
                GST / Tax{invoice?.gstCalculation?.gstRateLabel ? ` (${invoice.gstCalculation.gstRateLabel})` : invoice?.gstRate ? ` (${invoice.gstRate}%)` : ''}:
              </span>
