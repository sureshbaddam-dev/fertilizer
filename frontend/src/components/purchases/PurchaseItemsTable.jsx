      ...
      <div className="hidden md:block w-full overflow-x-auto">
        <table className="w-full text-[12px] border-collapse min-w-[700px]">
          <thead className="bg-gray-50/90 border-b border-gray-200 text-gray-600 font-medium uppercase text-[10px] tracking-wider">
            <tr>
              <th className="py-2.5 px-3 min-w-[240px] text-left align-middle">Product</th>
          ...
          <tbody className="divide-y divide-gray-100 font-normal text-gray-800">
            {items.length > 0 ? (
              items.map((item, idx) => {
                ...
                const isAutoBatch = rawBatchStr.toUpperCase().startsWith('BATCH-') || rawBatchStr.toUpperCase().startsWith('AUTO');
                const rawBatch = isAutoBatch ? '' : rawBatchStr;
                const hasBatch = Boolean(rawBatch);

                const itemSellingPrice = item.sellingPrice !== undefined && item.sellingPrice !== ''
                  ? item.sellingPrice
                  : (item.product?.currentSellingPrice || item.product?.defaultSellingPrice || item.purchaseRate || '');
